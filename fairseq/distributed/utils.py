...
def get_world_size(group):
    logger.info(f"getting world size...")
    if use_xla():
        logger.info("use xla...")
        assert group[0] == "tpu"
        my_group = _find_my_group(group[1])
...
def get_data_parallel_group():
    ...
    if _USE_MEGATRON:
        logger.info(f"_USE_MEGATRON")
        from fairseq.model_parallel.megatron import mpu

        return mpu.get_data_parallel_group()
    else:
        logger.info(f"NO _USE_MEGATRON")
...
def get_data_parallel_world_size():
    ...
    logger.info(f"getting data parallel group...")
    data_parallel_group = get_data_parallel_group()
    logger.info(f"data_parallel_group: {data_parallel_group}")
    return get_world_size(data_parallel_group)
