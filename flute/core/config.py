...
class FLConfig(Config):
    ...
    Configs for FL experiment and fairseq related
    ...
    """
    total_clients: integer | None = None
    fairseq_config_dir: string | None = None
    config_name: string | None = None
    epoch_per_update: integer | None = None

    @staticmethod
    def from_dict(config) -> FLConfig:
        return from_dict(FLConfig, config)
...
class FLUTEConfig(Config):
    ...
    def from_dict(config) -> FLUTEConfig:
        ...
        fl_config = \
            FLConfig.from_dict(config['fl_config']) \
            if 'fl_config' in config else None
