...
def _send_train_output(output):
    ...
    for key in output.keys():
        if key not in ['cs', 'pl']:
            print_rank(f"output key {key}: {output[key]}")
...
class Worker:
    ...
    def __init__(self, 
                fairseq_cfg=None, 
                task=None,               # fairseq task
                model=None,             # fairseq model
                criterion=None,     # fairseq criterion
                quantizer=None,  
                data_path=None, do_profiling=False, val_clients= None, 
    ...
    def run(self):
        ...
        while True:  # keeps listening for incoming server calls
            ...
            if command == COMMAND_UPDATE:
                ...
                # Get training data for client, including fairseq related attributes
                # TODO: Get corresponding data files (tsv files)
                fairseq_data= (self.fairseq_cfg, self.task, self.criterion, self.quantizer)

                # Execute Client.process_round()
                output = client_to_process.process_round(fairseq_data, client_data, server_data, self.model, self.data_path)
