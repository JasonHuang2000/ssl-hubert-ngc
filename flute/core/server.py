...
from .fairseq_train import fairseq_validate_and_save
...
class OptimizationServer(federated.Server):
    def __init__(self, num_clients, fairseq_cfg, task, model, \
                criterion, quantizer, optimizer, ss_scheduler, data_path, \
                model_path, server_train_dataloader, config, idx_val_clients, idx_test_clients):
        ...
        # The fairseq related attributes
        self.fairseq_cfg = fairseq_cfg
        self.task = task
        self.model = model
    ...
    def train(self):
        ...
        try:
            ...
            # save to "{token}_model.tar" format
            ...
            for i in range(self.cur_iter_no, self.max_iteration):
                ...
                print_rank('Clients for round: {}'.format(num_clients_curr_iter))
                ...
                fs_trainer = fairseq_trainer(self.fairseq_cfg, self.task, 
                                            self.worker_trainer.model, 
                                            self.criterion, self.quantizer)

                extra_state, epoch_itr = checkpoint_utils.load_checkpoint(
                    self.fairseq_cfg.checkpoint,
                    fs_trainer,
                    # don't cache epoch iterators for sharded datasets
                    disable_iterator_cache=self.task.has_sharded_data("train"),
                )
                valid_subsets = self.fairseq_cfg.dataset.valid_subset.split(",")

                # a dry run on validation set to pin the memory
                if not self.fairseq_cfg.dataset.disable_validation:
                    for subset in valid_subsets:
                        print('begin dry-run validation on "{}" subset'.format(subset))
                        itr = fs_trainer.get_valid_iterator(subset).next_epoch_itr(
                            shuffle=False, set_dataset_epoch=False  # use a fixed valid set
                        )
                        if self.fairseq_cfg.common.tpu:
                ...
                itr = epoch_itr.next_epoch_itr(
                    fix_batches_to_gpus=self.fairseq_cfg.distributed_training.fix_batches_to_gpus,
                    shuffle=(epoch_itr.next_epoch_idx > self.fairseq_cfg.dataset.curriculum),
                )

                valid_losses, should_stop = fairseq_validate_and_save(
                    self.fairseq_cfg, fs_trainer, self.task, 
                    epoch_itr, valid_subsets, end_of_epoch=True, fl_iteration=i
                )
