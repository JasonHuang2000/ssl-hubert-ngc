...
class Trainer(object):
    ...
    def data_parallel_world_size(self):
        if self.cfg.distributed_training.distributed_world_size == 1:
            return 1
        logger.info("data parallel world size...")
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    # logger.info(f"loss for training one step: {loss}, sample_size_i: {sample_size_i}")
                    total_loss += loss
                    del loss
        ...
        # logger.info(f"num of samples from fairseq trainer: {sample_size}")
        return logging_output, total_loss, sample_size
